function Check() {
    ...
    function max(x: number, y: number, ...numArray: Array<number>): void {
        console.log(Math.max(x, y, Math.max.apply(null, numArray)))
    }
    max(5, -2);
    max(5, -2, 30, 6);
    function getSqrt(...Num: (string | number | undefined)[]) {
        Num.length == 0 ?
...
const GetS = (sel: string) => document.querySelector(sel) as HTMLInputElement;
    ...
    if (GetS('.text-area').value == '') {
        ...
        for (let i = 0; i < cenzor!.length; i++) {
            if (GetS('.text-area').value.includes(cenzor![i])) {
                let text: string = GetS('.text-area').value
